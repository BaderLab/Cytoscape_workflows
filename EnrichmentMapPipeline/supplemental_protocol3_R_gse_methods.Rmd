...
if(!dataType_rnaseq){
  tryCatch(expr = { library("Biobase")}, 
    ...
    expressionMatrix <- as.matrix(read.table(
      file.path(working_dir, "Supplementary_Table1_TCGA_Microarray_rmanormalized.txt"), 
      header = TRUE, sep = "\t", quote="\"", stringsAsFactors = FALSE))
    classDefinitions <- read.table( 
      file.path(working_dir,"Supplementary_Table3_Microarray_classdefinitions.txt"), 
      header = TRUE, sep = "\t", quote="\"", stringsAsFactors = FALSE)
